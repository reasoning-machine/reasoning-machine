...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Reasoning-Machine thinks for you about the meanings.",
        epilog="Example:  reasoning-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | reasoning-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | reasoning-machine         #

    $ reasoning-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="reasoning-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"reasoning-machine: {utterance}.")
        syslog.closelog()
